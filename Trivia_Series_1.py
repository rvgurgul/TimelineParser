...
game_lights = loader.get_stat("json_outputs/game_info/sniper_lights.json")
...
spy_hilight_games = []
for g in game_lights:
    spy_char = game_casts[g]["Spy"]
    for light in game_lights[g][::-1]:
        if light["chara"] == spy_char:
            if light["light"] == 0:
                spy_lowlight_games.append(g)
            elif light["light"] == 2:
                spy_hilight_games.append(g)
            break
spy_lowlight_game_results = Counter([
    game_heads[g]["result"] for g in spy_lowlight_games
])
pie_chart(spy_lowlight_game_results)
spy_hilight_game_results = Counter([
    game_heads[g]["result"] for g in spy_hilight_games
])
pie_chart(spy_hilight_game_results)

# 6. Which spy has been lowlit the most?
spies_who_were_lowlit = Counter([
    game_heads[g]["spy_username"] for g in spy_lowlight_games
])
print(spies_who_were_lowlit.most_common())
